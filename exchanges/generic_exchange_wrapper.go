...
type ExchangeWrapper interface {
	...
	GetOrder(symbol string, orderId int64) (*futures.Order, error)
	GetBalances() (*futures.Account, error)
	...
	GetPositionRisk(market *environment.Market) ([]*futures.PositionRisk, error)
	...
	SetLeverage(market *environment.Market, leverage int) error
	SetHedgeMode(hedge bool) error
}
